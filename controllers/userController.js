...
exports.verifyOtpForDeliveryLogin = async (req, res) => {
  try {
    ...
    // Find the user by deliveryPartner.userId
    const user = await User.findByPk(deliveryPartner.userId);
    if (!user) {
      return res.status(404).json({ message: 'User not found for delivery partner' });
    }
